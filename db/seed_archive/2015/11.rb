...
"""
p.body = """
Ticket to Ride game plays like Connect 4 as played by plutocratic robber barons. A lot of the strategy comes from trying to play ambiguous moves that let you control territory without letting people know where you're going so they can't block you. It actually works pretty well as a two-player game too.
"""
p.asin = "0975277324"
p.cost = 32
...
"""
p.body = """
There are more expensive jigs out there, but the Grayling Fletching Jig is cheap, reliable, and gets the job done.  Just like your mom.  I've fletches hundreds of arrows with this jig. The only upkeep is in wiping off extra glue from the clamp so glue doesn't dry on it.
"""
p.asin = "B003DQVZ1Q"
p.cost = 40
...
"""
p.body = """
A simple circular saw lets you build an incredible number of things.  Take the DeWalt Circular Saw for a spin, it'll change you live.  If you need to cut a straight line you can clamp a rule or a board down then use it to guide the saw.
"""
p.asin = "B007QUZ16A"
p.cost = 129
...
"""
p.body = """
Useing the Guard to knock someone else out before they even have a chance to make a move is ridiculously satisfying and makes you feel like a goddamn oracle.  Love Letter is easy to learn and has quick rounds which makes it ideal for if you're waiting for something or aren't sure how long you'll be able to play.
"""
p.asin = "B00AGJ4HC2"
p.cost = 9
p.tag_list.add(TagType::TOYS, TagType::BOARDGAMES)
...
"""
p.body = """
This game is like NSFW Apples to Apples. It's a great party game that scales to large groups, but use discretion since it's not for everyone. I like to play the Rando Cardrissian modification, where you deal cards to an imaginary player and randomly pick one every round. It's great fun to talk trash to people who are worse than random chance. Another house rule that speeds up the game is to ignore the last card that is played every round.
"""
p.asin = "B004S8F7QM"
p.cost = 25
p.tag_list.add(TagType::TOYS, TagType::BOARDGAMES)
